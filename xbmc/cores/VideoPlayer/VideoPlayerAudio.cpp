    ...
    m_messageQueueTimeSize(messageQueueTimeSize)
{
  ...
  m_messageQueue.SetMaxDataSize(32 * m_messageQueueTimeSize / 8 * 1024 * 1024);
...
void CVideoPlayerAudio::UpdatePlayerInfo()
{
  ...
  std::ostringstream s;
  s << "aq:" << std::setw(2) << std::min(99, level) << "% (" << std::setw(2) << std::min(99,m_messageQueue.GetLevel(true)) << "%)";
